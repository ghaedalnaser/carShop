...
export const getBy = async (key?: string, value?: string): Promise<Part[]> => {
    ...
    let query = `SELECT p.*,m.make,m.model_name ,u.phone_number as phon ,s.email as store_email,s.name as store_name, s.facebook as store_facebook,s.location as store_location FROM public."part" as p join public."model" as m on p.model_id=m.id join public."store" as s on p.store_id=s.id join public."user" as u on u.id = s.user_id` ;
