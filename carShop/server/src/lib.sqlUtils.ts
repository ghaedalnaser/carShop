...
export const generateInserteQuery = <T>(
  tableName: string,
  defaultObject: T,
  insertObject: Partial<T>,
  returnRow = true,
  deleteId = true
) => {
...
export const generateUpdateQuery = <T>(
  ...
  if (deleteId && "id" in cleaned) delete cleaned["id"];
  if (!images) {
    if ("images" in cleaned) delete cleaned["images"];
    if ("photo" in cleaned) delete cleaned["photo"];
  }
...
export const generateDeleteQueryForAgency = <T>(
  tableName: string,
  criterias: any
) => {
  let paramCounter = 0;
